<template>
  <div class="flex h-screen w-full overflow-hidden" style="background-color: var(--bg-app)">
    <!-- Sidebar (desktop) -->
    <aside class="hidden w-60 shrink-0 flex-col border-r border-white/10 bg-brand-blue-950/95 backdrop-blur-xl lg:flex">
      <RouterLink to="/dashboard" class="flex shrink-0 items-center gap-2 px-5 py-5">
      ...
      <button
        type="button"
        class="mx-3 mb-4 flex items-center gap-3 rounded-xl px-3 py-2.5 text-sm font-semibold text-white/75 transition hover:bg-red-500/15 hover:text-red-300"
        @click="handleLogout"
      >
